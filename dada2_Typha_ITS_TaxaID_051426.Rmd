...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(170,70),
                     minLen=60, maxN=0, maxEE=c(2,4), truncQ=2,
...
## Identify number of bimeras out of how many input sequences.
...
write.csv(track, "C:/Users/saysh/OneDrive/Desktop/Kuehn_Final_Analyses/Typha/Typha_Fungi/Typha_Fungi_Rerun_March2026/dada2_Typha_ITS_read_tracking_041426.csv")
...
write(asv_fasta, "dada2_Typha_ITS_fasta_042126.fasta")

#Generate ASV table (.count_table file)
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(t(asv_tab), "dada2_Typha_ITS_asvtable_042126.count_table", sep="\t", quote=F, col.names=NA)

#Generate taxonomy table
rownames(tax_info) <- gsub(pattern=">", replacement="", x=asv_headers)
write.csv(tax_info, "dada2_Typha_ITS_taxonomy_042126.csv")
...
fas <- "dada2_Typha_ITS_fasta_042126.fasta"
...
write.csv(idtax_table_commonizing, "Typha_ITS_idtax_tab_DECIPHER_051126.csv")
```

## Remove fungal kingdoms 
```{r}
# Reload dada2 taxonomy if needed
tax_info <- read.csv("dada2_Typha_ITS_taxonomy_042126.csv", row.names=1)
...
write.csv(eTax1, "Typha_ITS_ensembleTax_tab_051126.csv")
```
...
write.csv(fungitax, "Typha_final_fungi_taxtab_051126.csv")

full_euktax<- filter(eTax1, !is.na(Kingdom))

write.csv(full_euktax, "Typha_final_ITS_eukaryote_taxtab_051126.csv")
```
